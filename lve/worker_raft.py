...
from settings import raft_core_directory

sys.path.append(raft_core_directory)
from raft import RAFT

class WorkerRaft(Worker):
    def __init__(self, w, h, c, fps, ins, options):
        super().__init__(w, h, c, fps, options)  # do not forget this
        self.raft_small = options['net']['architecture'] == 'sota-raft-small'

        self.optical_flow = lve.OpticalFlowCV(backward=False)
        self.model = RAFT(Namespace(small=self.raft_small, mixed_precision=False))
        self.ins = ins
        self.__frame = None
    ...
    def call_raft(self, frame, old_frame):
        _, flow_up = self.model(old_frame, frame, iters=20, test_mode=True)
        flow_np = flow_up.detach().squeeze().permute(1, 2, 0).cpu().numpy()
        return flow_up, flow_np
    ...
    def process_frame(self, frame, of=None, supervisions=None, foa=None):
        ...
        flow, flow_np = self.call_raft(frame * 255.0, old_frame * 255.0)
    ...
    def load(self, model_folder):
        ...
        model.load_state_dict(torch.load(model_folder + ("raft-small.pth" if self.raft_small else "raft-sintel.pth"),
